...
const {ROLE_ADMIN, ROLE_peternak, capitalize, getMessage, validateRole, createRedisClient} = require('../utils.js');
...
/**
 * @param  {Request} req Body must be a peternak json and role in the header
 * @param  {Response} res 201 response if asset is created else 400 with a simple json message
 * @description Creates a peternak as an user adds the peternak to the wallet
 */
...
exports.getAllPatients = async (req, res) => {
  // User role from the request header is validated
  const userRole = req.headers.role;
  await validateRole([ROLE_ADMIN, ROLE_peternak], userRole, res);
  // Set up and connect to Fabric Gateway using the username in header
  const networkObj = await network.connectToNetwork(req.headers.username);
  // Invoke the smart contract function
  const response = await network.invoke(networkObj, true, capitalize(userRole) + 'Contract:queryAllPatients',
    userRole === ROLE_peternak ? req.headers.username : '');
