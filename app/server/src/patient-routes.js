...
const {ROLE_ADMIN, ROLE_peternak, ROLE_PATIENT, capitalize, getMessage, validateRole} = require('../utils.js');
...
exports.getPatientById = async (req, res) => {
  ...
  await validateRole([ROLE_peternak, ROLE_PATIENT], userRole, res);
 ...
 * @param  {Response} res 200 response with array of all peternak else 500 with the error message
 * @description Get all the peternak of the mentioned hospitalId
 */
exports.getpeternaksByHospitalId = async (req, res) => {
  // User role from the request header is validated
  const userRole = req.headers.role;
...
/**
 * @param  {Request} req Role in the header. patientId, peternakId in the url
 * @param  {Response} res 200 response if access was granted to the peternak else 500 with the error message
 * @description Patient grants access to the peternak.
 */
exports.grantAccessTopeternak = async (req, res) => {
  // User role from the request header is validated
  const userRole = req.headers.role;
  await validateRole([ROLE_PATIENT], userRole, res);
  const patientId = req.params.patientId;
  const peternakId = req.params.peternakId;
  let args = {patientId: patientId, peternakId: peternakId};
  args= [JSON.stringify(args)];
  // Set up and connect to Fabric Gateway
  const networkObj = await network.connectToNetwork(req.headers.username);
  // Invoke the smart contract function
  const response = await network.invoke(networkObj, false, capitalize(userRole) + 'Contract:grantAccessTopeternak', args);
  (response.error) ? res.status(500).send(response.error) : res.status(200).send(getMessage(false, `Access granted to ${peternakId}`));
};
/**
 * @param  {Request} req Role in the header. patientId, peternakId in the url
 * @param  {Response} res 200 response if access was revoked from the peternak else 500 with the error message
 * @description Patient revokes access from the peternak.
 */
exports.revokeAccessFrompeternak = async (req, res) => {
  // User role from the request header is validated
  const userRole = req.headers.role;
  await validateRole([ROLE_PATIENT], userRole, res);
  const patientId = req.params.patientId;
  ...
  const networkObj = await network.connectToNetwork(req.headers.username);
  // Invoke the smart contract function
  const response = await network.invoke(networkObj, false, capitalize(userRole) + 'Contract:revokeAccessFrompeternak', args);
  (response.error) ? res.status(500).send(response.error) : res.status(200).send(getMessage(false, `Access revoked from ${peternakId}`));
};
