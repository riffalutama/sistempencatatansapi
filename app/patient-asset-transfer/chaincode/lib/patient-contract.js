...
class PatientContract extends PrimaryContract {
     ...
     * @param  {Context} ctx
     * @param  {JSON} args containing patientId and peternakId
     * @description Add the peternak to the permissionGranted array
     */
    async grantAccessTopeternak(ctx, args) {
        args = JSON.parse(args);
        let patientId = args.patientId;
        let peternakId = args.peternakId;

        // Get the patient asset from world state
        const patient = await this.readPatient(ctx, patientId);
        // unique peternakIDs in permissionGranted
        if (!patient.permissionGranted.includes(peternakId)) {
            patient.permissionGranted.push(peternakId);
            patient.changedBy = patientId;
        }
        const buffer = Buffer.from(JSON.stringify(patient));
     ...
     * @param  {Context} ctx
     * @param  {JSON} args containing patientId and peternakId
     * @description Remove the peternak from the permissionGranted array
     */
    async revokeAccessFrompeternak(ctx, args) {
        args = JSON.parse(args);
        let patientId = args.patientId;
        let peternakId = args.peternakId;
        ...
        // Get the patient asset from world state
        const patient = await this.readPatient(ctx, patientId);
        // Remove the peternak if existing
        if (patient.permissionGranted.includes(peternakId)) {
            patient.permissionGranted = patient.permissionGranted.filter(peternak => peternak !== peternakId);
            patient.changedBy = patientId;
        }
        const buffer = Buffer.from(JSON.stringify(patient));
        // Update the ledger with updated permissionGranted
