...
class peternakContract extends AdminContract {
    ...
    async readPatient(ctx, patientId) {
        ...
        // Get the peternakID, retrieves the id used to connect the network
        const peternakId = await this.getClientId(ctx);
        // Check if peternak has the permission to read the patient
        const permissionArray = asset.permissionGranted;
        if(!permissionArray.includes(peternakId)) {
            throw new Error(`The peternak ${peternakId} does not have permission to patient ${patientId}`);
        }
        asset = ({
            patientId: patientId,
    ...
    //This function is to update patient medical details. This function should be called by only peternak.
    ...
    async queryAllPatients(ctx, peternakId) {
        let resultsIterator = await ctx.stub.getStateByRange('', '');
        let asset = await this.getAllPatientResults(resultsIterator, false);
        const permissionedAssets = [];
        for (let i = 0; i < asset.length; i++) {
            const obj = asset[i];
            if ('permissionGranted' in obj.Record && obj.Record.permissionGranted.includes(peternakId)) {
...
module.exports = peternakContract;