 ...
 * @description Connects to the network using the username - peternakID, networkObj contains the paramters using which
 * @description a connection to the fabric network is possible.
 */
exports.connectToNetwork = async function(peternakID) {
  ...
  try {
    ...
    const userExists = await wallet.get(peternakID);
    if (!userExists) {
      console.log('An identity for the peternakID: ' + peternakID + ' does not exist in the wallet');
      console.log('Create the peternakID before retrying');
      const response = {};
      response.error = 'An identity for the user ' + peternakID + ' does not exist in the wallet. Register ' + peternakID + ' first';
      return response;
    }
    ...
    */
    ...
    await gateway.connect(ccp, {wallet, identity: peternakID, discovery: {enabled: true, asLocalhost: true}});
 ...
 * @description Creates a patient/peternak and adds to the wallet to the given hospitalId
 */
 ...
 * @return {JSON} Returns an JSON array consisting of all peternak object.
 * @description Retrieves all the users(peternak) based on user type(peternak) and hospitalId
 */
exports.getAllpeternaksByHospitalId = async function(networkObj, hospitalId) {
  ...
  try {
    ...
    for (let i = 0; i < identities.length; i++) {
      ...
      if (identities[i].type === 'client') {
        ...
        // peternak object will consist of firstName and lastName
    ...
    console.error(`Unable to get all peternak : ${error}`);
    const response = {};
    response.error = error;
    return response;
  }
